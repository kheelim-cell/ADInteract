  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-semibold text-gray-900 group-hover:{t.label} transition-colors flex-1">{m.investor_tool_districts_title()}</span>
        <svg class="w-3.5 h-3.5 text-gray-300 group-hover:text-amber-600 transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
        </svg>
      </div>
      <p class="text-xs text-gray-500 leading-snug">{m.investor_tool_districts_desc()}</p>
    </a>
