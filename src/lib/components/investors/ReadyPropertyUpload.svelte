<script lang="ts">
  export type ReadyExtractionData = {
  ...
  type Props = {
    onExtracted: (data: ReadyExtractionData) => void;
  };
  ...
  async function extractOne(file: File): Promise<ReadyExtractionData> {
    ...
    return json.data as ReadyExtractionData;
  }

  // First non-null value wins across all results
  function mergeResults(results: ReadyExtractionData[]): ReadyExtractionData {
    const merged: ReadyExtractionData = {
      district: null, layout: null, project: null, price: null,
      livingArea: null, balconyArea: null, serviceChargePsf: null, annualRent: null,
    };
    for (const r of results) {
      for (const key of Object.keys(merged) as (keyof ReadyExtractionData)[]) {
  ...
  async function handleFiles(files: File[]) {
    ...
    const results: ReadyExtractionData[] = [];
...
<div class="rounded-xl border border-white/15 bg-white/3 overflow-hidden">
  ...
  {#if status === 'idle' || status === 'error'}
    ...
    <label
      class="flex flex-col items-center gap-2.5 px-5 py-5 cursor-pointer transition-colors
        {isDragging ? 'bg-emerald-500/10 border-emerald-500/40' : 'hover:bg-white/4'}"
    ...
    >
      ...
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-xl bg-emerald-500/15 flex items-center justify-center flex-shrink-0">
          <svg class="w-4.5 h-4.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
          </svg>
        </div>
        <div>
          <p class="text-xs font-semibold text-white/80">
            {isDragging ? m.upload_drop_to_scan() : m.upload_ready_title()}
          </p>
  ...
  {:else if status === 'processing'}
    ...
    <div class="flex items-center gap-3 px-5 py-4">
      <svg class="w-4 h-4 text-emerald-400 animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24">
            ...
            {#if src}
              <img
                ...
                class="w-8 h-8 rounded-md object-cover border flex-shrink-0
                  {i + 1 === currentFile ? 'border-emerald-400' : 'border-white/10 opacity-40'}"
              />
            {/if}
          {/each}
        </div>
